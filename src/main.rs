...
fn main() {
    ...
    let mut universe = Universe::new(80, 40);
    universe.rand();
    ...
    let mut per_instance = {
        ...
        implement_vertex!(Attr, alive);

        let data = (0..3200).map(|_| {
            Attr {
                alive: 1.0,
            }
        }).collect::<Vec<_>>();
    ...
    vec3 computed = vec3(float(mod(gl_InstanceID,80)) - 40.0, float(gl_InstanceID/80) - 20.0, 0);
    
    void main() {
        vnormal = transpose(inverse(mat3(matrix))) * normal;
        vec4 new_position = alive * matrix * vec4(position, scaling);
    ...
    event_loop.run(move |ev, _, control_flow| {
        ...
        *control_flow = glutin::event_loop::ControlFlow::WaitUntil(next_frame_time);
...

        {
            ...
            for (id, attr) in (0..3200).zip(mapping.iter_mut()) {
                attr.alive = { match universe.alive(id) {
                    true => 1.0,
                    false => 0.0
                }}
            }
        ...
        */
        ...
        .draw(
            ...
            &index_buffer,
            &program,
            &uniform! { scaling: cube.scaling, matrix: matrix, perspective: perspective, light: light },
            &params,
        )
        .unwrap();
